...
class Graph:
    ...
    def create_graph(self):
        ...
        pos = {
            ...
            1: (0, 0), 2: (-0.75, 1), 3: (-0.75, 2), 4: (0, 3), 5: (0.75, 2), 6: (0.75, 1),\
            # bottom row, second from the left
            7: (1.5, 3), 8: (2.25, 2), 9: (2.25, 1), 10: (1.5, 0),\
            # bottom row, third from the left
            11: (3, 3), 12: (3.75, 2), 13: (3.75, 1), 14: (3, 0), 'A': (0, 1.5), 'B': (1.5, 1.5), 'C': (3, 1.5), \
        ...
        hexagons_polygons = {
            1: {"1": [0, -0.75, -0.75, 0, 0.75, 0.75],
                       "2": [0, 1, 2, 3, 2, 1]},
            2: {"1": [1.5, 0.75, 0.75, 1.5, 2.25, 2.25],
                      "2": [0, 1, 2, 3, 2, 1]},
            3: {"1": [3, 2.25, 2.25, 3, 3.75, 3.75],
                      "2": [0, 1, 2, 3, 2, 1]}

                             }

        for x in hexagons_polygons:
            plt.fill("1", "2", updated_nodes.dict_hex_colors[x], alpha=0.5, edgecolor= None, lw=2, ls= 'solid', zorder=0, data=hexagons_polygons[x])
        ...
        def on_click(event):
            ...
            if event.button is MouseButton.LEFT and self.__dict__['option'] == 'settlement':
                ...
                for i in pos:
                    if pos[i] == coordinates_tuple and color_map[i-1] == 'lightskyblue' and coordinates[0] != None\
                            and updated_nodes.dict_prop[i] != 'settlement_player_2' and updated_nodes.dict_prop[i] !='city_player_2':
                        color_map_red.append('red')
                    elif pos[i] == coordinates_tuple and color_map[i-1] != 'lightskyblue'and coordinates[0] == None:
                        color_map_red.append('white')
                    elif pos[i] == coordinates_tuple and color_map[i-1] != 'lightskyblue'and coordinates[0] != None:
                        color_map_red.append('white')
                    elif pos[i] != coordinates_tuple and type(i) == int:
                        color_map_red.append('white')
                    elif pos[i] != coordinates_tuple and type(i) == str:
                        color_map_red.append('yellow')
                ...
                for node in G:
                    if type(node) == int:
                        if updated_nodes.dict_prop[node] == 'settlement_player_1':
                            ...
                            color_map_red[node - 1] = ('brown')
                        elif updated_nodes.dict_prop[node] == 'city_player_2':
                            color_map_red[node - 1] = ('grey')
            ...
            if event.button is MouseButton.LEFT and self.__dict__['option'] == 'city':
                ...
                color_map_red = []
                for i in pos:
                    if pos[i] == coordinates_tuple and color_map[i-1] == 'green' and coordinates[0] != None:
                        color_map_red.append('red')
                    if pos[i] == coordinates_tuple and color_map[i-1] == 'green' and coordinates[0] == None:
                        color_map_red.append('white')
                    elif pos[i] == coordinates_tuple and color_map[i-1] != 'green':
                        color_map_red.append('white')
                    elif pos[i] != coordinates_tuple and type(i) == int:
                        color_map_red.append('white')
                    elif pos[i] != coordinates_tuple and type(i) == str:
...
class Graph_Properties:
    ...
    def __init__(self, turn):
        ...
        #bogus dictionary with colors ofthe hexes (dependent on the resources associated with them)
        self.dict_hex_colors = {1:'black', 2:'yellow', 3:'yellow'}
